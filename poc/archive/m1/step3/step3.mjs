// Step 3, x (was goal 2 until 2026-09-13): truth x, predicted w — the
// leak; live-verb then yours-noun.
//
...
// checks and their word lists belong to step 3 (see lists.mjs,
...
// only" comparison can compose floor -> this step -> step 1's read-verb
...
// the pipeline). ctx = { junkSet, allowlistFor } from step3/allowlist.mjs's
// buildStep3Context(); allowlistFor(vendor) returns that vendor's LOVO
// yours-noun allowlist.
export function applyStep3(prev, row, ctx) {
