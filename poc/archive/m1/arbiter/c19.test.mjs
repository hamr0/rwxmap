...
// --- Part A: the combined corpus load ---------------------------------

test('loadCombinedCorpus loads the original 1478 rows plus exam2 and exam3, dropping truth_class "?"', () => {
  const { allRows, originalRows, exam2Rows, exam3Rows, exam2Dropped, exam2Total, exam3Dropped, exam3Total } = loadCombinedCorpus();
  ...
  assert.equal(exam3Total, 3000);
  assert.equal(exam3Dropped, 7);
  assert.equal(exam3Rows.length, exam3Total - exam3Dropped);
  assert.equal(allRows.length, originalRows.length + exam2Rows.length + exam3Rows.length);
});

test('every original row carries confidence "high"; exam2/exam3 rows carry the labeller\'s own value', () => {
  const { originalRows, exam2Rows, exam3Rows } = loadCombinedCorpus();
  assert.ok(originalRows.every((r) => r.confidence === 'high'));
  assert.ok(exam2Rows.every((r) => r.confidence === 'high' || r.confidence === 'low'));
  assert.ok(exam3Rows.every((r) => r.confidence === 'high' || r.confidence === 'low'));
  assert.ok(exam2Rows.some((r) => r.confidence === 'low'), 'expected at least one low-confidence exam2 row');
  assert.ok(exam3Rows.some((r) => r.confidence === 'low'), 'expected at least one low-confidence exam3 row');
});

test('no exam2/exam3 row has truth_class "?"', () => {
  const { exam2Rows, exam3Rows } = loadCombinedCorpus();
  assert.ok(exam2Rows.every((r) => r.gt_class === 'r' || r.gt_class === 'w' || r.gt_class === 'x'));
  assert.ok(exam3Rows.every((r) => r.gt_class === 'r' || r.gt_class === 'w' || r.gt_class === 'x'));
});

test('camara rows all share vendor "camara"; exam2/exam3 rows use their own provider as vendor', () => {
  ...
  const exam3Rows = allRows.filter((r) => r.set === 'exam3');
  const exam3Vendors = new Set(exam3Rows.map((r) => r.vendor));
  assert.ok(exam3Vendors.size > 0, 'expected at least one exam3 vendor');
});
...
// if this fails, no "after" number in c19-sweep.md can be trusted.
...
// --- deriveAndAdmit (Part E's learning-curve building block) -------------

test('deriveAndAdmit admits a noun that qualifies and spans enough vendors, on a synthetic corpus', () => {
  const rows = [
    row({ vendor: 'vendorA', method: 'DELETE', operationId: 'deleteWidget', summary: 'Delete a widget', gt_class: 'x' }),
    row({ vendor: 'vendorB', method: 'DELETE', operationId: 'deleteWidget', summary: 'Delete a widget', gt_class: 'x' }),
    row({ vendor: 'vendorC', method: 'DELETE', operationId: 'deleteWidget', summary: 'Delete a widget', gt_class: 'x' }),
    row({ vendor: 'vendorD', method: 'DELETE', operationId: 'deleteWidget', summary: 'Delete a widget', gt_class: 'x' }),
  ];
  const alreadyKnown = new Set([...PARTY_NOUNS, ...SHARED_NOUNS]);
  const result = deriveAndAdmit(rows, alreadyKnown);
  assert.ok(result.qualifying.some((c) => c.noun === 'widget'));
  assert.ok(result.survivors.some((c) => c.noun === 'widget'));
});

test('deriveAndAdmit excludes an already-known noun from survivors even when it qualifies and survives LOVO', () => {
  const rows = [
    row({ vendor: 'vendorA', method: 'DELETE', operationId: 'deleteAccount', summary: 'Delete an account', gt_class: 'x' }),
    row({ vendor: 'vendorB', method: 'DELETE', operationId: 'deleteAccount', summary: 'Delete an account', gt_class: 'x' }),
    row({ vendor: 'vendorC', method: 'DELETE', operationId: 'deleteAccount', summary: 'Delete an account', gt_class: 'x' }),
  ];
  const alreadyKnown = new Set([...PARTY_NOUNS, ...SHARED_NOUNS]);
  assert.ok(alreadyKnown.has('account'));
  const result = deriveAndAdmit(rows, alreadyKnown);
  assert.ok(result.qualifying.some((c) => c.noun === 'account'));
  assert.ok(!result.survivors.some((c) => c.noun === 'account'));
});
