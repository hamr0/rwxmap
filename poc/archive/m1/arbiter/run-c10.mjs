// M1-C10: the two-pass shape (adds summaryVerbOn to the C9 judge).
//
...
const OUT_ROWS = path.join(REPO_ROOT, 'docs/logs/m1/c10-rows.csv');
const OUT_FALSE_FLAGS = path.join(REPO_ROOT, 'docs/logs/m1/c10-false-flags.csv');
const OUT_SWEEP = path.join(REPO_ROOT, 'docs/logs/m1/c10-sweep.md');
const OUT_MISSES = path.join(REPO_ROOT, 'docs/logs/m1/c10-judge-misses.md');
...
function main() {
  ...
  const rawSwitches = { r1On: true, r2On: true, r3On: true, judgePostOwn: true, pathPartyOn: false, liveTokenOn: false, summaryVerbOn: false, repoNounOn: false, floorOn: false };
  ...
  // M1-C10b fix 2: repoNounOn decided second, holding liveTokenOn at its
  // chosen value and summaryVerbOn/pathPartyOn at their OFF baselines — it
  // feeds R2 (and, through R2 admission, which rows even reach R3), so it
  // is decided before summaryVerbOn is re-measured.
  const repoNounOff = evaluateSourceSwitches({ pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: false, repoNounOn: false });
  const repoNounOn = evaluateSourceSwitches({ pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: false, repoNounOn: true });
  console.log('\nM1-C10b fix 2: R2 "repository" party-noun source, repoNounOn OFF vs ON (admitted rules, floorOn=false, camara+holdout1):');
  console.log(fmtRows(
    [
      ['off', repoNounOff.reviewSum, repoNounOff.overTightSum, repoNounOff.cost, [...repoNounOff.rejected].join(',') || 'none'],
      ['on', repoNounOn.reviewSum, repoNounOn.overTightSum, repoNounOn.cost, [...repoNounOn.rejected].join(',') || 'none'],
    ],
    ['repoNounOn', 'review', 'over_tight', 'cost', 'rejected'],
  ));
  const repoNounOnChosen = repoNounOn.cost < repoNounOff.cost; // tie -> off
  console.log(`Chosen: repoNounOn=${repoNounOnChosen} (off cost ${repoNounOff.cost}, on cost ${repoNounOn.cost}).`);

  // M1-C10b: summaryVerbOn RE-measured third, now with fix 1 (caller-phrase
  // suppression, unconditional, not a measured switch) and fix 2
  // (repoNounOn) already in place — holding liveTokenOn and repoNounOn at
  // their chosen values, pathPartyOn at its OFF baseline.
  const summaryVerbOff = evaluateSourceSwitches({ pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: false, repoNounOn: repoNounOnChosen });
  const summaryVerbOn = evaluateSourceSwitches({ pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: true, repoNounOn: repoNounOnChosen });
  console.log('\nM1-C10b: R2/R3/R4 summary-verb fallback source, RE-MEASURED with fixes 1+2 in place, summaryVerbOn OFF vs ON (admitted rules, floorOn=false, camara+holdout1):');
  console.log(fmtRows(
    [
      ['off', summaryVerbOff.reviewSum, summaryVerbOff.overTightSum, summaryVerbOff.cost, [...summaryVerbOff.rejected].join(',') || 'none'],
      ['on', summaryVerbOn.reviewSum, summaryVerbOn.overTightSum, summaryVerbOn.cost, [...summaryVerbOn.rejected].join(',') || 'none'],
    ],
    ['summaryVerbOn', 'review', 'over_tight', 'cost', 'rejected'],
  ));
  const summaryVerbOnChosen = summaryVerbOn.cost < summaryVerbOff.cost; // tie -> off
  console.log(`Chosen: summaryVerbOn=${summaryVerbOnChosen} (off cost ${summaryVerbOff.cost}, on cost ${summaryVerbOn.cost}).`);

  // --- coordinator request: if R3 still leaks under summaryVerbOn ON (raw,
  // before rejection), list every leaking row, then print — CLEARLY
  // LABELLED, NOT USED FOR ANY DECISION — the numbers summaryVerbOn=ON
  // would give if those specific rows were excluded from the leak count
  // (i.e. R3 admitted, and those rows' actual leak outcome subtracted from
  // the reported leak tally, as a placeholder for a pending truth re-read).
  const summaryVerbOnRawTable = summaryVerbOn.rawTable;
  const r3LeaksUnderSummaryVerbOn = leaksOnAdmitSets(summaryVerbOnRawTable, 'R3');
  console.log(`\nR3 leaking rows under summaryVerbOn=ON (raw, camara+holdout1): ${r3LeaksUnderSummaryVerbOn.length}`);
  for (const { row, result } of r3LeaksUnderSummaryVerbOn) {
    console.log(`  ${row.set}/${row.repo} ${row.method} ${row.operationId || '(empty)'} gt=${row.gt_class} pred=${result.class} evidence=[${result.evidence.join(' ; ')}]`);
  }
  let hypotheticalRows = null;
  if (r3LeaksUnderSummaryVerbOn.length > 0) {
    const excludeKeys = new Set(r3LeaksUnderSummaryVerbOn.map(({ row }) => [row.set, row.repo, row.path, row.method, row.operationId].join('|')));
    const hypoRejected = new Set();
    for (const rule of RULES) {
      const leakingRows = leaksOnAdmitSets(summaryVerbOnRawTable, rule).filter(({ row }) => !excludeKeys.has([row.set, row.repo, row.path, row.method, row.operationId].join('|')));
      if (leakingRows.length > 0) hypoRejected.add(rule);
    }
    const hypoSwitches = {
      r1On: !hypoRejected.has('R1'), r2On: !hypoRejected.has('R2'), r3On: !hypoRejected.has('R3'), judgePostOwn: !hypoRejected.has('R4'),
      pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: true, repoNounOn: repoNounOnChosen, floorOn: false,
    };
    const hypoPass2Off = scorePass2(pass1Review, hypoSwitches);
    const hypoPass2On = scorePass2(pass1Review, { ...hypoSwitches, floorOn: true });
    function hypoSummary(pass2, floorLabel) {
      return SETS.map((s) => {
        const real = finalSummary(pass1Scored, pass2, s);
        const excludedInSet = [...excludeKeys].filter((k) => k.split('|')[0] === s).length;
        return { ...real, leaks: real.leaks - excludedInSet, exact: real.exact + excludedInSet, note: `floorOn=${floorLabel}, ${excludedInSet} row(s) excluded from leak count` };
      });
    }
    hypotheticalRows = { rejected: hypoRejected, floorOff: hypoSummary(hypoPass2Off, false), floorOn: hypoSummary(hypoPass2On, true) };
    console.log('\nHYPOTHETICAL, PENDING THE USER\'S TRUTH RE-READ — not used for any decision. summaryVerbOn=ON with the listed R3-leaking rows excluded from the leak count:');
    console.log(`  rules rejected under this hypothetical: ${hypoRejected.size ? [...hypoRejected].join(',') : 'none'}`);
    console.log('  floorOn=false:');
    console.log('  ' + fmtRows(hypotheticalRows.floorOff.map((r) => [r.set, r.n, r.assigned, r.review, r.leaks, r.overTight, r.exact]), ['set', 'n', 'assigned', 'review', 'leaks', 'over_tight', 'exact']).split('\n').join('\n  '));
    console.log('  floorOn=true:');
    console.log('  ' + fmtRows(hypotheticalRows.floorOn.map((r) => [r.set, r.n, r.assigned, r.review, r.leaks, r.overTight, r.exact]), ['set', 'n', 'assigned', 'review', 'leaks', 'over_tight', 'exact']).split('\n').join('\n  '));
  }

  // pathPartyOn decided fourth, holding liveTokenOn, repoNounOn and
  // summaryVerbOn at their chosen values.
  const pathPartyOff = evaluateSourceSwitches({ pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: summaryVerbOnChosen, repoNounOn: repoNounOnChosen });
  const pathPartyOn = evaluateSourceSwitches({ pathPartyOn: true, liveTokenOn: liveTokenOnChosen, summaryVerbOn: summaryVerbOnChosen, repoNounOn: repoNounOnChosen });
  ...
  const rawRuleTablePathPartyOnText = ruleTableText(perRuleTable(scorePass2(pass1Review, { ...rawSwitches, pathPartyOn: true, liveTokenOn: liveTokenOnChosen, summaryVerbOn: summaryVerbOnChosen, repoNounOn: repoNounOnChosen })));
  const rawRuleTablePathPartyOffText = ruleTableText(perRuleTable(scorePass2(pass1Review, { ...rawSwitches, pathPartyOn: false, liveTokenOn: liveTokenOnChosen, summaryVerbOn: summaryVerbOnChosen, repoNounOn: repoNounOnChosen })));
  ...
  const rawRuleTableRepoNounOnText = ruleTableText(perRuleTable(scorePass2(pass1Review, { ...rawSwitches, repoNounOn: true, liveTokenOn: liveTokenOnChosen, pathPartyOn: false })));
  const rawRuleTableRepoNounOffText = ruleTableText(perRuleTable(scorePass2(pass1Review, { ...rawSwitches, repoNounOn: false, liveTokenOn: liveTokenOnChosen, pathPartyOn: false })));
  const rawRuleTableSummaryVerbOnText = ruleTableText(perRuleTable(scorePass2(pass1Review, { ...rawSwitches, summaryVerbOn: true, liveTokenOn: liveTokenOnChosen, repoNounOn: repoNounOnChosen, pathPartyOn: false })));
  const rawRuleTableSummaryVerbOffText = ruleTableText(perRuleTable(scorePass2(pass1Review, { ...rawSwitches, summaryVerbOn: false, liveTokenOn: liveTokenOnChosen, repoNounOn: repoNounOnChosen, pathPartyOn: false })));
  ...
  // --- write c9-sweep.md -----------------------------------------------------
  const smd = [];
  smd.push('# M1-C10 — the two-pass shape: pass 1 (byte-for-byte C7, layer3On/layer4On both true) + pass 2 judge');
  smd.push('');
  ...
  smd.push('```');
  smd.push('');
  smd.push(`### M1-C10b fix 2: R2 "repository" party-noun source, repoNounOn OFF vs ON (decided second, liveTokenOn held at ${liveTokenOnChosen})`);
  smd.push('');
  smd.push('```');
  smd.push(fmtRows(
    [
  ...
  smd.push('```');
  smd.push('');
  smd.push(`Chosen: repoNounOn=${repoNounOnChosen} (off cost ${repoNounOff.cost} vs on cost ${repoNounOn.cost}).`);
  smd.push('');
  smd.push('Full per-rule tables, repoNounOn off vs on (liveTokenOn held at the chosen value, pathPartyOn held off):');
  smd.push('```');
  smd.push('-- repoNounOn=false --');
  smd.push(rawRuleTableRepoNounOffText);
  smd.push('-- repoNounOn=true --');
  smd.push(rawRuleTableRepoNounOnText);
  smd.push('```');
  smd.push('');
  smd.push(`### M1-C10b: R2/R3/R4 summary-verb fallback source, RE-MEASURED with fix 1 (caller-phrase suppression) and fix 2 (repoNounOn=${repoNounOnChosen}) in place, summaryVerbOn OFF vs ON (decided third, liveTokenOn held at ${liveTokenOnChosen}, pathPartyOn held off)`);
  smd.push('');
  smd.push('```');
  smd.push(fmtRows(
    [
  ...
  smd.push('```');
  smd.push('');
  smd.push(`Chosen: summaryVerbOn=${summaryVerbOnChosen} (off cost ${summaryVerbOff.cost} vs on cost ${summaryVerbOn.cost}).`);
  smd.push('');
  smd.push('Full per-rule tables, summaryVerbOn off vs on (liveTokenOn/repoNounOn held at their chosen values, pathPartyOn held off):');
  smd.push('```');
  smd.push('-- summaryVerbOn=false --');
  smd.push(rawRuleTableSummaryVerbOffText);
  smd.push('-- summaryVerbOn=true --');
  smd.push(rawRuleTableSummaryVerbOnText);
  smd.push('```');
  smd.push('');
  smd.push(`R3 leaking rows under summaryVerbOn=ON (raw, camara+holdout1): ${r3LeaksUnderSummaryVerbOn.length}`);
  smd.push('');
  if (r3LeaksUnderSummaryVerbOn.length > 0) {
    smd.push('```');
    for (const { row, result } of r3LeaksUnderSummaryVerbOn) {
      smd.push(`${row.set}/${row.repo} ${row.method} ${row.operationId || '(empty)'} gt=${row.gt_class} pred=${result.class} evidence=[${result.evidence.join(' ; ')}]`);
    }
    smd.push('```');
    smd.push('');
    smd.push('**HYPOTHETICAL, PENDING THE USER\'S TRUTH RE-READ — not used for any decision.** summaryVerbOn=ON with the rows listed just above excluded from the leak count (i.e. R3 re-admitted if that was its only reason for rejection here, and those rows\' leak counted as if resolved):');
    smd.push('');
    smd.push(`Rules rejected under this hypothetical: ${hypotheticalRows.rejected.size ? [...hypotheticalRows.rejected].join(',') : 'none'}`);
    smd.push('');
    smd.push('```');
    smd.push('-- floorOn=false --');
    smd.push(fmtRows(hypotheticalRows.floorOff.map((r) => [r.set, r.n, r.assigned, r.review, r.leaks, r.overTight, r.exact]), finalHeader));
    smd.push('-- floorOn=true --');
    smd.push(fmtRows(hypotheticalRows.floorOn.map((r) => [r.set, r.n, r.assigned, r.review, r.leaks, r.overTight, r.exact]), finalHeader));
    smd.push('```');
  } else {
    smd.push('None — summaryVerbOn=ON reached zero R3 leaks on camara/holdout1 with fixes 1+2 in place, no hypothetical needed.');
  }
  smd.push('');
  smd.push(`### Correction 2: R2 path-party-id source, pathPartyOn OFF vs ON (decided fourth, liveTokenOn held at ${liveTokenOnChosen}, repoNounOn held at ${repoNounOnChosen}, summaryVerbOn held at ${summaryVerbOnChosen})`);
  smd.push('');
  smd.push('```');
  smd.push(fmtRows(
    [
  ...
  smd.push('```');
  smd.push('');
  smd.push(`## Rule admission decisions (>=1 leak on camara or holdout1 rejects a rule, at the chosen source switches: pathPartyOn=${pathPartyOnChosen}, liveTokenOn=${liveTokenOnChosen}, summaryVerbOn=${summaryVerbOnChosen})`);
  smd.push('');
  if (rejectionLines.length === 0) {
  ...
  const mmd = [];
  mmd.push('# M1-C10 — pass-2 misses (over-tight and leaks) at the chosen (admitted+floorOn) configuration');
  mmd.push('');
