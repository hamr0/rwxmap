...
|---|---|---|
| `poc/m1/core/` | floor table, verb rules, noun extraction, corpus + allowlist setup | — (shared; changes need every pin green) |
...
4. **Each goal's number is pinned by its own test**
   (`run/ledger.test.mjs`): goal 2 = 37, goal 1 = 2727, goal 3 = 49. A
...
That cost was charged to goal 1 when goal 2 was frozen. Goal 1 is next, as a lower-back layer in `poc/m1/goal1/` with its own evidence and its own leak ledger.
