...
By confidence:
  ...
  - method-only: {"agree":8,"over-tight":12}
...
| ApplicationProfiles | DELETE | /application-profiles/{applicationProfileId} | deleteApplicationProfile | x | policy | L5-floor-x | verb=delete | w | The 204 response description "Application profile has been deleted successfully" indicates a delete whose repeated effect (profile absent) does not change. |
...
| ConnectedNetworkType | DELETE | /subscriptions/{subscriptionId} | deleteConnectedNetworkTypeSubscription | x | policy | L5-floor-x | verb=delete | w | This is the standard CAMARA subscription delete operation on /subscriptions/{subscriptionId}: it removes the identified subscription, converging to the same absent-subscription end state on repetition and issuing no further notifications itself. |
...
| DeviceRoamingStatus | DELETE | /subscriptions/{subscriptionId} | deleteDeviceRoamingStatusSubscription | x | policy | L5-floor-x | verb=delete | w | deleteDeviceRoamingStatusSubscription deletes "a given device-roaming-status subscription by ID," stopping only the consumer's own future notifications with no third-party effect. |
| EdgeApplicationManagement | DELETE | /apps/{appId} | deleteApp | x | policy | L5-floor-x | verb=delete | w | description 'Delete all the information and content related to an Application' idempotently removes the caller's own application record. |
...
| NetworkAccessManagement | DELETE | /trust-domains/{trustDomainId} | deleteTrustDomain | x | policy | L5-floor-x | verb=delete | w | Deletes the Trust Domain and removes its configuration from all network access devices where applied, a standard idempotent resource delete. |
...
| NetworkAccessManagement | DELETE | /trust-domains/{trustDomainId}/devices/{deviceId} | deleteTrustDomainDevice | x | policy | L5-floor-x | verb=remove | w | Deregisters and removes the device from the specified Trust Domain, an idempotent delete of the registration resource. |
