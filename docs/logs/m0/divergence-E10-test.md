...
By method:
  ...
  - DELETE: {"agree":13,"over-tight":10}
  - PATCH: {"over-tight":2,"agree":1}

By confidence:
  - method-only: {"agree":16,"over-tight":6}
  - high: {"agree":87,"over-tight":20}
  - low: {"agree":19,"over-tight":3,"WRONG-LOOSENING":1}
...
| BlockchainPublicAddress | DELETE | /blockchain-public-addresses/{id} | deleteBlockchainPublicAddress | x | low | L3-live-noun | nouns=access verb=unbind | w | Description says it "Unbinds Blockchain Public Address to a mobile phone number" for the given bind id, a deletion whose end state (unbound) is stable under repetition. |
...
| DeviceStatus | DELETE | /subscriptions/{subscriptionId} | deleteConnectedNetworkTypeSubscription | x | high | L3-live-noun | nouns=network verb=delete | w | Deletes the caller's own subscription (204/202 async), a delete-resource operation whose repetition converges to the same deleted state. |
