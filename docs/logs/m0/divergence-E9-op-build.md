...
By method:
  - POST: {"agree":57,"over-tight":10}
  - PATCH: {"over-tight":4,"agree":2}
  - GET: {"agree":46}
  - DELETE: {"agree":4,"over-tight":14}
  - PUT: {"agree":1,"over-tight":2}

By confidence:
  - low: {"agree":29,"over-tight":8}
  - high: {"agree":77,"over-tight":19}
  - method-only: {"agree":4,"over-tight":3}
...
|---|---|---|---|---|---|---|---|---|---|
| ApplicationProfiles | PATCH | /application-profiles/{applicationProfileId} | updateApplicationProfile | x | low | L3-live-noun | nouns=network verb=update | w | Description states it updates "the complete set of network quality thresholds for an application with the new set of thresholds", i.e. a full replace of the identified profile's thresholds. |
...
| IoTSIMFraudPrevention | POST | /query | query | x | low | L3-live-noun | nouns=device verb=iot | r | The request body only carries a device identifier and queryType, and the response only reports current bind or area-limit status with no resource created or modified. |
...
| NetworkAccessManagement | PATCH | /trust-domains/{trustDomainId} | updateTrustDomain | x | low | L3-live-noun | nouns=access,device,network verb=update | w | Updates the Trust Domain by ID with full-replacement semantics for accessDetails, so repeating the same PATCH body leaves the domain in the same state. |
| NetworkAccessManagement | DELETE | /trust-domains/{trustDomainId} | deleteTrustDomain | x | high | L3-live-noun | nouns=access,device,network verb=delete | w | Deletes the Trust Domain and removes its configuration from all network access devices where applied, a standard idempotent resource delete. |
| NetworkAccessManagement | PATCH | /trust-domains/{trustDomainId}/devices/{deviceId} | updateTrustDomainDevice | x | low | L3-live-noun | nouns=device verb=update | w | Only included fields are updated, with `bootstrappingInfo`/`deviceCredential` replaced wholesale when present, making repeated identical PATCHes converge on the same device state. |
| NetworkAccessManagement | DELETE | /trust-domains/{trustDomainId}/devices/{deviceId} | deleteTrustDomainDevice | x | low | L3-live-noun | nouns=device verb=remove | w | Deregisters and removes the device from the specified Trust Domain, an idempotent delete of the registration resource. |
...
| OptimalEdgeDiscovery | POST | /retrieve-optimal-edge-cloud-zones | discoverOptimalEdge | x | low | L3-live-noun | nouns=device verb=discover | r | Request body is only an applicationProfileId plus optional device/region identifiers, and the response is a ranked list of zones with no resource created by the call. |
...
| WebRTC | DELETE | /sessions/{registrationId} | deleteRegistrationById | x | high | L3-live-noun | nouns=device,network,session verb=delete | w | Deleting "frees any resource and invalidates future actions that involves this registration" — an idempotent removal of the caller's own registration resource with no external effect stated for this operation. |
