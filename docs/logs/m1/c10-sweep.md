# M1-C10 — the two-pass shape: pass 1 (byte-for-byte C7, layer3On/layer4On both true) + pass 2 judge
...
### M1-C10b fix 2: R2 "repository" party-noun source, repoNounOn OFF vs ON (decided second, liveTokenOn held at true)
...
Chosen: repoNounOn=false (off cost 139 vs on cost 139).

Full per-rule tables, repoNounOn off vs on (liveTokenOn held at the chosen value, pathPartyOn held off):
```
-- repoNounOn=false --
rule  set       hit  exact  leaks  over_tight
----  --------  ---  -----  -----  ----------
...
### M1-C10b: R2/R3/R4 summary-verb fallback source, RE-MEASURED with fix 1 (caller-phrase suppression) and fix 2 (repoNounOn=false) in place, summaryVerbOn OFF vs ON (decided third, liveTokenOn held at true, pathPartyOn held off)
...
Chosen: summaryVerbOn=false (off cost 139 vs on cost 211).

Full per-rule tables, summaryVerbOn off vs on (liveTokenOn/repoNounOn held at their chosen values, pathPartyOn held off):
```
-- summaryVerbOn=false --
rule  set       hit  exact  leaks  over_tight
----  --------  ---  -----  -----  ----------
...
R1    holdout2  11   10     0      1         
R2    camara    14   7      0      7         
R2    holdout1  46   18     0      28        
R2    holdout2  16   10     0      6         
R3    camara    35   35     0      0         
R3    holdout1  95   92     3      0         
R3    holdout2  48   48     0      0         
R4    camara    3    0      3      0         
R4    holdout1  22   16     6      0         
R4    holdout2  18   4      13     1         
```

R3 leaking rows under summaryVerbOn=ON (raw, camara+holdout1): 3

```
holdout1/github DELETE repos/delete gt=x pred=w evidence=[judge:own:summary:repository]
holdout1/github PUT issues/set-issue-field-values gt=x pred=w evidence=[judge:own:summary:field]
holdout1/github DELETE issues/remove-sub-issue gt=x pred=w evidence=[judge:own:summary:issue]
```

**HYPOTHETICAL, PENDING THE USER'S TRUTH RE-READ — not used for any decision.** summaryVerbOn=ON with the rows listed just above excluded from the leak count (i.e. R3 re-admitted if that was its only reason for rejection here, and those rows' leak counted as if resolved):

Rules rejected under this hypothetical: R4
...
holdout1  207  185       22      0      32          153  
holdout2  220  185       35      0      8           177  
-- floorOn=true --
set       n    assigned  review  leaks  over_tight  exact
--------  ---  --------  ------  -----  ----------  -----
camara    292  292       0       0      20          272  
holdout1  207  207       0       0      48          159  
holdout2  220  219       1       0      22          197  
```

### Correction 2: R2 path-party-id source, pathPartyOn OFF vs ON (decided fourth, liveTokenOn held at true, repoNounOn held at false, summaryVerbOn held at false)
...
## Rule admission decisions (>=1 leak on camara or holdout1 rejects a rule, at the chosen source switches: pathPartyOn=false, liveTokenOn=true, summaryVerbOn=false)
